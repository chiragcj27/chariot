  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      addItem({
        ...
        price: 80, // Mock price - matches backend pricing
        creditsCost: 80, // Mock credits cost - matches backend pricing
        imageUrl: kit.mainImage?.url || kit.thumbnail?.url,
      });
      
      // Show success message
      alert('Kit added to cart successfully!');
    } catch (error) {
      ...
      <section>
        ...
        <div className="flex overflow-x-auto gap-4 md:gap-6 pb-4 px-5 md:px-10 lg:px-18 scrollbar-hide">
          {sampleProducts.map((product) => (
            <div
              key={product.id}
              className="flex-shrink-0 w-48 sm:w-56 md:w-70 h-64 sm:h-72 md:h-100 bg-white rounded-lg shadow-md overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-xl"
