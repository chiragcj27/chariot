...
export async function GET(req: NextRequest) {
  try {
    ...
    const baseBackendUrl = process.env.BACKEND_API_URL || 'http://localhost:3001';
    const backendUrl = `${baseBackendUrl}/api/marketplace/settings`;
    ...
    console.error('Error in marketplace settings GET:', error);
    return NextResponse.json(
...
export async function PUT(req: NextRequest) {
  try {
    ...
    console.error('Error in marketplace settings PUT:', error);
    return NextResponse.json(
      { message: 'Internal server error' },
      { status: 500 }
    );
