...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!accessToken) {
      ...
      if (refreshToken) {
        console.log('Kit Main Files POST - No access token, attempting refresh...');
        ...
        if (backendRefresh.ok) {
          ...
          console.log('Kit Main Files POST - Token refreshed successfully');
        }
      }
      
      if (!accessToken) {
        console.log('Kit Main Files POST - No valid token available, refresh failed');
    ...
    const backendUrl = process.env.BACKEND_API_URL || 'http://localhost:3001/api/files/kit-main-files';
    ...
    console.error('Kit main files error:', error);
    return NextResponse.json(
