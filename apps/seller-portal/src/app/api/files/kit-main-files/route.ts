...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!accessToken) {
      ...
      if (refreshToken) {
        console.log('Kit main files - No access token, attempting refresh...');
        ...
        if (backendRefresh.ok) {
          ...
          console.log('Kit main files - Token refreshed successfully');
        }
      }
      
      if (!accessToken) {
        console.log('Kit main files - No valid token available, refresh failed');
    ...
    const baseBackendUrl = process.env.BACKEND_API_URL || 'http://localhost:3001';
    const backendUrl = `${baseBackendUrl}/api/files/kit-main-files`;
