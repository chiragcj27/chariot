...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!accessToken) {
      ...
      if (refreshToken) {
        console.log('Kit preview files - No access token, attempting refresh...');
        ...
        if (backendRefresh.ok) {
          ...
          console.log('Kit preview files - Token refreshed successfully');
        }
      }
      
      if (!accessToken) {
        console.log('Kit preview files - No valid token available, refresh failed');
    ...
    const baseBackendUrl = process.env.BACKEND_API_URL || 'http://localhost:3001';
    const backendUrl = `${baseBackendUrl}/api/files/kit-preview-files`;
