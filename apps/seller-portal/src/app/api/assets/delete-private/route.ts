...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!accessToken) {
      ...
      if (refreshToken) {
        console.log('Assets DELETE PRIVATE - No access token, attempting refresh...');
        ...
        if (backendRefresh.ok) {
          ...
          console.log('Assets DELETE PRIVATE - Token refreshed successfully');
        }
      }
      
      if (!accessToken) {
        console.log('Assets DELETE PRIVATE - No valid token available, refresh failed');
    ...
    const backendUrl = process.env.BACKEND_API_URL || 'http://localhost:3001/api/assets/delete-private';
    ...
    console.error('Private asset deletion error:', error);
    return NextResponse.json(
